...
#define AIC_USB_VENDOR_ID_LEGACY	0xa69c
#define AIC_USB_VENDOR_ID_V2	0x368b
#define AIC_USB_PRODUCT_ID_D81	0x8d81
...
MODULE_PARM_DESC(injections, "Number of AIC D80 ACL bulk OUT URBs modified");
...
static bool is_aic_d80_runtime(const struct usb_device *udev)
{
	u16 vendor;
	...
	return le16_to_cpu(udev->descriptor.idProduct) ==
		       AIC_USB_PRODUCT_ID_D81 &&
	       (vendor == AIC_USB_VENDOR_ID_LEGACY ||
		vendor == AIC_USB_VENDOR_ID_V2);
}

static bool is_aic_bulk_out(const struct urb *urb)
{
	if (!urb || !is_aic_d80_runtime(urb->dev))
		return false;
...
static void enable_zlp(struct urb *urb)
{
	...
	if (count == 1)
		pr_info("enabled ZLP on the first AIC D80 ACL bulk OUT URB\n");
}
...
static int __init aic_zlp_quirk_init(void)
{
	...
	if (!ret) {
		...
		pr_info("attached to system btusb for AIC D80 runtime devices\n");
		return 0;
	}
	...
	pr_info("attached USB submit fallback for AIC D80 Bluetooth interface 0\n");
	return 0;
}
