                                ...
                                u32 boot_type)
{
    ...
    if (diag_d80_handoff && usbdev &&
        ...
        printk("d80_handoff: DBG_START_APP_REQ build chipid=%u bootaddr=0x%08x boottype=%u req_cfm=0 cfm_id=0 wait_cfm=0\n",
               usbdev->chipid, boot_addr, boot_type);
    ...
    if (!start_app_req) {
        if (diag_d80_handoff && usbdev &&
            (usbdev->chipid == PRODUCT_ID_AIC8800D80 ||
             usbdev->chipid == PRODUCT_ID_AIC8800D80X2))
            printk("d80_handoff: DBG_START_APP_REQ alloc_failed chipid=%u ret=%d\n",
                   usbdev->chipid, -ENOMEM);
        return -ENOMEM;
    }
    ...
    if (diag_d80_handoff && usbdev &&
        (usbdev->chipid == PRODUCT_ID_AIC8800D80 ||
         usbdev->chipid == PRODUCT_ID_AIC8800D80X2))
        printk("d80_handoff: DBG_START_APP_REQ send_done chipid=%u ret=%d req_cfm=0 cfm_id=0 app_ready_confirmed=0\n",
               usbdev->chipid, ret);
